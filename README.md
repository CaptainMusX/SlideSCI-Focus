...
- **「生成放大图」**：弹出参数设置窗口，一键生成：
  - 放大目标：与原图相同尺寸 / 指定放大倍数 / 自定义宽度(cm)
  - 连线样式：**期刊漏斗**（框下两角 → 放大图上两角，平行非同角直连）或**交叉 X 形**（四角交叉）
  - 连线/框线：颜色（含自定义）、粗细、实线/虚线
  - 实时预览放大图尺寸与实际放大倍数，比例不一致时给出拉伸警告
- **连线自动吸附**：连线端点通过 PowerPoint 连接点粘附到选区框四角与放大图四角（隐形角锚点），
  之后移动图片或调整位置，连线自动跟随拉伸，不需要重新计算
- **像素无损**：全程只做复制、裁剪、缩放、连线、编组等矢量操作，不导出文件、不重新编码图像，
  原图像素原封不动
- 重复生成自动覆盖旧放大图与连线，选区框保留
