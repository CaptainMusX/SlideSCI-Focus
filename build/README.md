...
- `artifacts\SlideSCI-<版本>\setup.exe`
- `artifacts\SlideSCI-<版本>\CaptainMusX.SlideSCI.vsto`
- `artifacts\SlideSCI-<版本>\Application Files\`
- `artifacts\SlideSCI-<版本>\SHA256SUMS.txt`

默认不把 MathJax 的 `node_modules` 放进发布目录，以保持与原作者 Release 一致；如需离线 LaTeX SVG 运行时，可显式运行 `-BundleLatexRuntime`。

本地没有配置正式代码签名证书时，脚本会在当前用户的 `%LOCALAPPDATA%\SlideSCI\build-signing` 中生成开发用自签名证书。它适合本机测试，正式分发前应替换为受信任的发布证书，并保留同一签名身份用于后续更新。
...
该安装器参考原作者 Advanced Installer 的直接布局：把 `bin\Release` 中的 `CaptainMusX.SlideSCI.vsto`、`CaptainMusX.SlideSCI.dll.manifest`、程序集和 `latex-converter` 文件放在用户选择的目录，排除 `app.publish`、`Application Files` 和 `node_modules`，并按应用清单中的 `keyName="CaptainMusX.SlideSCI"` 注册加载项。构建时会从已签名应用清单提取公钥，安装时只为当前清单 URL 与该公钥写入用户级 VSTO Inclusion 信任，卸载时同步移除。安装前会清理原版与 fork 版的旧 VSTO URL、ClickOnce 元数据和历史注册项；不会运行旧的 ClickOnce `setup.exe`，因此不会复用已删除的 D 盘清单。卸载入口由 Inno Setup 提供。

安装前清理已安装版本（脚本会先备份注册项并保护仓库目录）：

```powershell
powershell -ExecutionPolicy Bypass -File .\build\Clean-SlideSCI-Install.ps1
```
